...
void LCD_ShowChar32(u16 x,u16 y,u8 index,u8 mode)
{
    u16 j,k;
    u8 temp,t1;
    u16 x1=x;
    //char:[32][3]
    for(k=index*32; k<index*32+32; k++)
    {
        for(j=0; j<3; j++)
        {
            temp = seg_3224[k][j];
...
void LCD_ShowChar48(u16 x,u16 y,u8 index,u8 mode)
{
    u16 j,k;
    u8 temp,t1;
    u16 x1=x;
    //char:[48][4]
    for(k=index*48; k<index*48+48; k++)
    {
        for(j=0; j<4; j++)
        {
            temp = seg_4832[k][j];
...
void LCD_ShowChar64(u16 x,u16 y,u8 index,u8 mode)
{
    u16 j,k;
    u8 temp,t1;
    u16 x1=x;
    //char:[54][6]
    for(k=index*54; k<index*54+54; k++)
    {
        for(j=0; j<6; j++)
        {
            temp = seg_5448[k][j];
...
void LCD_ShowNum48(u16 x,u16 y,u32 num,u8 len)
{         	
	u8 t,temp;
	//u8 enshow=0;		
	u8 enshow=1;    
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				//LCD_ShowChar48(x+32*t,y,0,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar48(x+32*t,y,temp,0); 
	}
} 
void LCD_ShowNum64(u16 x,u16 y,u32 num,u8 len)
{         	
	u8 t,temp;
	//u8 enshow=0;		
	u8 enshow=1;    
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				//LCD_ShowChar64(x+48*t,y,0,0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar64(x+48*t,y,temp,0); 
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
void LCD_ShowStopWatch(u16 x,u16 y,u8 mode,u16 fore_color,u16 back_color)
{
    u16 j,k;
    u8 temp,t1;
    u16 x1=x;
    for(k=0; k<64; k++)
    {
        for(j=0; j<8; j++)
        {
            ...
            for(t1=0;t1<8;t1++)
            {
                if(temp&0x80)       LCD_Fast_DrawPoint(x,y,fore_color);
                else if(mode==0)    LCD_Fast_DrawPoint(x,y,back_color);
                temp<<=1;
                x++;
            }
